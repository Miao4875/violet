...
from core.interfaces import AgentFactory, HistoryBackend, LoggerBackend, ModelProvider, ToolFactory
...
class Registry:
    ...
    def register_history(self, name: str, backend_cls: type[HistoryBackend]) -> None:
        if name in self.history_backends:
            raise ValueError(f"history backend '{name}' already registered")
    ...
    def register_logger(self, name: str, backend_cls: type[LoggerBackend]) -> None:
        if name in self.logger_backends:
            raise ValueError(f"logger backend '{name}' already registered")
    ...
    def register_tool(self, name: str, factory: ToolFactory) -> None:
    ...
    def get_tool(self, name: str) -> ToolFactory:
        if name not in self.tool_factories:
            raise KeyError(f"tool '{name}' is not registered")
        return self.tool_factories[name]
